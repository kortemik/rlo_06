...
public class PriorityImpl implements Priority {
    ...
    public PriorityImpl(Facility facility, Severity severity) {
        this(new FragmentImpl(Collections.singletonList(ByteBuffer.wrap(new byte[] {
                '<'
        }))
        ),
                new FragmentImpl(
                        Collections.singletonList(ByteBuffer.wrap(String.valueOf(facility.value() * 8 + severity.value()).getBytes(StandardCharsets.US_ASCII)))
                ),
                new FragmentImpl(Collections.singletonList(ByteBuffer.wrap(new byte[] {
                        '>'
                })))
        );
    }
